...
export class SelectHotelComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formGroup.controls[ControlName.City].valueChanges
      .pipe(
        debounceTime(300),
        switchMap((value) => this.hotelService.getHotels(value || '')),
      )
      .subscribe((result) => (this.hotels = result));
  }

  bookHotel(): void {
    this.router.navigate([
      'bookhotel',
      this.formGroup.controls[ControlName.Hotel].value,
    ]);
  }
}
