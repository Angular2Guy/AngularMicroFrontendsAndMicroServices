...
export class BookFlightComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.flightService
      .getFlightById(this.activatedRoute.snapshot.params['id'])
      .subscribe((result) => (this.selFlight = result));
    this.bookingService
      .getAllBookings()
  ...
  protected bookFlight(): void {
    !!this.selFlight?.id &&
      this.bookingService
        .postBooking(this.selFlight.id, {
          id: null,
          flightDate:
            this.formGroup.controls[ControlName.Day].value?.toISOString(),
        } as Booking)
        .pipe(mergeMap(() => this.bookingService.getAllBookings()))
        .subscribe((result) => (this.bookings = result));
  }
